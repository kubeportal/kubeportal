...
class ApiTestCase(AdminLoggedOutTestCase):
    ...
    def api_login(self):
        response = self.post(F'/api/{settings.API_VERSION}/login', {'username': admin_data['username'], 'password': admin_clear_password})
        self.assertEqual(response.status_code, 200)
...
class ApiAnonymous(ApiTestCase):
    ...
    def test_api_wrong_login(self):
        response = self.post(F'/api/{settings.API_VERSION}/login', {'username': admin_data['username'], 'password': 'blabla'})
        self.assertEqual(response.status_code, 400)

    def test_cluster_denied(self):
        for stat in ClusterViewSet.stats.keys():
            with self.subTest(stat=stat):
                response = self.get(F'/api/{settings.API_VERSION}/cluster/{stat}')
    ...
    def test_webapp_denied(self):
        ...
        response = self.get(F'/api/{settings.API_VERSION}/webapps/{app1.pk}')
        self.assertEqual(response.status_code, 401)
    ...
    def test_user_webapps_denied(self):
        ...
        response = self.get(F'/api/{settings.API_VERSION}/users/{self.admin.pk}/webapps')
        self.assertEqual(response.status_code, 401)

    def test_group_denied(self):
        response = self.get(F'/api/{settings.API_VERSION}/groups/{self.admin_group.pk}')
        self.assertEqual(response.status_code, 401)

    def test_user_groups_denied(self):
        response = self.get(F'/api/{settings.API_VERSION}/users/{self.admin.pk}/groups')
        self.assertEqual(response.status_code, 401)

    def test_logout(self):
        # logout should work anyway, even when nobody is logged in
        response = self.post(F'/api/{settings.API_VERSION}/logout')
        self.assertEqual(response.status_code, 200)
    ...
    def test_invalid_google_login(self):
        ...
        '''
        response = self.post(F'/api/{settings.API_VERSION}/login_google', {'access_token': 'foo', 'code': 'bar'})
        self.assertEqual(response.status_code, 400)
...
class ApiLocalUser(ApiTestCase):
    ...
    def test_cors_multiple_allowed(self):
        headers = {'Origin': 'http://testserver'}
        response = self.get(f'/api/{settings.API_VERSION}/cluster/portal_version', headers=headers)
        self.assertEqual(
            response.headers['Access-Control-Allow-Origin'], 'http://testserver')

    def test_cluster_invalid(self):
        response = self.get(F'/api/{settings.API_VERSION}/cluster/foobar')
    ...
    def test_webapp_invalid_id(self):
        response = self.get(F'/api/{settings.API_VERSION}/webapps/777')
        self.assertEqual(response.status_code, 404)
    ...
    def test_user_webapps_invalid_id(self):
        response = self.get(F'/api/{settings.API_VERSION}/users/777/webapps')
        self.assertEqual(response.status_code, 403)
    ...
    def test_group_invalid_id(self):
        response = self.get(F'/api/{settings.API_VERSION}/groups/777')
        self.assertEqual(response.status_code, 404)
    ...
    def test_group_non_member(self):
        ...
        response = self.get(F'/api/{settings.API_VERSION}/groups/{group1.pk}')
        self.assertEqual(response.status_code, 403)

    def test_group_invalid_id_by_user(self):
        response = self.get(F'/api/{settings.API_VERSION}/users/777/groups')
        self.assertEqual(response.status_code, 403)

    def test_user(self):

        response = self.get(F'/api/{settings.API_VERSION}/users/{self.admin.pk}')
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_patch_user_invalid_id(self):
        response = self.patch(F'/api/{settings.API_VERSION}/users/777', {})
        self.assertEqual(response.status_code, 404)
    ...
    def test_patch_user_not_himself(self):
        ...
        response = self.patch(F'/api/{settings.API_VERSION}/users/{u.pk}', {})
        self.assertEqual(response.status_code, 403)

    def test_user_invalid_id(self):
        response = self.get(F'/api/{settings.API_VERSION}/users/777')
        self.assertEqual(response.status_code, 404)
    ...
    def test_user_not_himself(self):
        ...
        response = self.get(F'/api/{settings.API_VERSION}/users/{u.pk}')
        self.assertEqual(response.status_code, 403)

    def test_no_general_user_list(self):
        response = self.get(F'/api/{settings.API_VERSION}/users')
        self.assertEqual(response.status_code, 404)

    def test_no_general_webapp_list(self):
        response = self.get(F'/api/{settings.API_VERSION}/webapps')
        self.assertEqual(response.status_code, 404)

    def test_no_general_group_list(self):
        response = self.get(F'/api/{settings.API_VERSION}/groups')
        self.assertEqual(response.status_code, 404)
